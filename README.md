...
1. **关节状态停滞问题解决**：
   - 问题分析：离线数据验证过程中某些关节预测值收敛到固定状态，与真实曲线出现较大差异，可能导致实际执行时机器人动作不自然
   - 解决方法：自适应loss权重、不同肢体数据归一化范围缩放
   - 效果：所有关节在离线数据验证时能正常响应并产生如预期的动作序列
...
3. **实验结果分析**：
   - 最佳方法：不同肢体数据归一化范围缩放
   - 性能提升：提高了模型对精细动作的预测准确性，减小验证误差

4. **实机测试经验**
    - 尝试多种action horizon的采样方式对预测的动作序列采样执行，尝试了截取不同阶段的动作进行执行，以及进行降采样；由于动作停滞问题，采用靠后动作执行更可能执行出连续推进的动作而减少在某个阶段不正常的停滞
    - 可视化机器人动作与状态曲线，DP可以通过图像信息做出对任务阶段的判断并做出相应动作决策，但这种视觉观测给出的反馈并不灵敏，机器人动作经常停滞与某一阶段无法继续向下执行
    - 目标物摆放位置与机器人自身状态在实际执行时都会对整体执行效果产生影响，当目标处于高度感兴趣区域时视觉反馈可能会推动policy进入下一动作阶段；但机器人本身的状态如果处于一个不相干状态（即并非抓取预备状态或与数据初始状态不符）有可能成为policy推进决策的阻力，policy可能没有学习到如何将当前状态推进到熟悉的区域
...
1. **训练稳定性优化**：
   - 训练不稳定问题：训练时梯度范数不断增大，但loss会在开始猛烈下降并在后续保持平缓，在训练集的采样样本上loss保持较低，但在完整episoid验证时loss很高，预测的action完全无序
   - 采用AdamW优化器替代原有Adam，提供更好的权重正则化
   - 对梯度小的注意力模块引入权重衰减（weight decay=1e-4）进行正则化
   - 添加残差链接，改善梯度流动和特征传递
   - 注册梯度钩子函数，记录训练时各模块的梯度信息进行具体分析
   - 以上改动稳定了训练与验证效果，离线验证曲线效果与DP类似或更好

2. **训练效果改进**：
   - 训练损失曲线下降更平稳，波动减小
    ![](./media/gradient_analysis_step_92200.png)

   - 验证集性能提升明显，平均误差降低
   ![](./media/example_test_results/predicted_trajectory.png)
    ![](./media/example_test_results/target_trajectory.png)
   - 模型收敛速度提升，梯度范数异常的模块减少
    ![](./media/grad_distribution_step_92200.png)


3. **实机测试分析**：
   - 实机测试出现初始状态敏感的情况，即如果机器人初始关节状态如果不能与policy训练时的初始状态相似，模型会推理出混乱的动作；且在符合初始状态的情况下出现对训练数据动作的过拟合，重复地执行动作而视觉反馈无效
   - 使用不同噪声预测方式训练模型，得到的模型实机执行时状况类似
   - 根本原因分析：训练数据分布与实际执行环境存在差异导致过拟合的模型无法适应，而视觉condition的作用明显不如DP在实机上表现出的性能
   - 解决方向：增加数据增强和环境随机化，改进数据分布；修改模型结构尤其是condition方式，权衡视觉信息与拟合action之间的平衡，尽可能发挥Diffusionmodel的多样化生成能力，使模型能有效使用视觉观测并据此推出模仿的专家动作
