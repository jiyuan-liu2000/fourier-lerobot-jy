...
def load_path_from_hdf5(hdf5_path: Path):
    video_path = hdf5_path.parent / hdf5_path.stem / "top" / "rgb.mp4"
    timestamps_path = hdf5_path.parent / hdf5_path.stem / "top" / "timestamps.json"
    return video_path, timestamps_path
...
def get_match_idx(frame_timestamps_path, data_ts):
    import json
    with open(frame_timestamps_path, 'r') as f:
        timestamps = json.load(f)
        frame_ts = np.array([iso_to_datetime(ts).timestamp() for ts in timestamps], dtype=np.float64)
...
def load_from_raw(
    ...
    for ep_idx in tqdm.tqdm(ep_ids):
        ...
        with h5py.File(ep_path, "r") as ep:
            video_path, camera_timestamps_path = load_path_from_hdf5(ep_path)
            robot_timestamps = ep["/timestamp"][:]
            matched_ts = get_match_idx(camera_timestamps_path, robot_timestamps)
